...
O TLV é um protocolo completo, seguro e autónomo. O CAN FD constitui exclusivamente o canal de transporte. O CRC e a integridade do CAN são complementares ao CRC8 e HMAC do TLV, não substitutos.
...
* segurança por diseño (CRC8, HMAC, anti-replay);
...
| 0xA4    | FLD_FS_TIMEOUT  | uint32_t    | Timeout da condição de failsafe (ms) |
...
| 0xF3    | CMD_SET_HOME        | lat, lon, alt           | Definir posição de回归 (home) |
...
| 11    | FS_PILOT_INPUT     | Input do piloto inválido |
| 12    | FS_MANUAL_OVERRIDE| Override manual |
...
Overhead CAN FD = DLC + CAN ID + flags ≈ 8-12 bytes (depende da implementação)

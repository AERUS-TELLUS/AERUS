...
| ------------- | ----- | --------------------------------------------------- |
| Prioridade    | 28-26 | 0=SuperCrítico, 1=Crítico, 2=Alto, 3=Médio, 4=Baixo |
| Grupo Origem  | 25-22 | ID do grupo que envia a mensagem                    |
| Grupo Destino | 21-18 | ID do grupo que recebe (0x0 = difusão)              |
...
| Reservado   | 0x6-0xF | Expansão futura        |
...
| 0x8-0xF | Reservado       | Expansão futura                |
...
| ---------- | ------------ | ------------- | ------ | ---------- | ----------------------------- |
| 0x01000000 | RaspberryPi  | Difusão       | DATA   | ALTO       | Telemetria para todos         |
| 0x01100000 | RaspberryPi  | ESP32-S       | CMD    | ALTO       | Comandos para sensores        |
| 0x01200000 | RaspberryPi  | ESP32-A       | CMD    | CRÍTICO    | Comandos de voo               |
| 0x01300000 | RaspberryPi  | ESP32-FS      | DATA   | ALTO       | Estados para segurança        |
| 0x01600000 | RaspberryPi  | Difusão       | HEART  | MÉDIO      | Heartbeat RaspberryPi         |
| 0x01400000 | RaspberryPi  | Difusão       | EVENT  | ALTO       | Eventos de missão             |
| 0x02000000 | ESP32-S_01   | Difusão       | DATA   | ALTO       | Sensores frente               |
| 0x02040000 | ESP32-S_01   | ESP32-FS      | DATA   | ALTO       | Sensores frente → segurança   |
| 0x02100000 | ESP32-S_01   | RaspberryPi   | DATA   | ALTO       | Sensores frente → RPi         |
| 0x02200000 | ESP32-S_02   | Difusão       | DATA   | ALTO       | Sensores traseira             |
| 0x02240000 | ESP32-S_02   | ESP32-FS      | DATA   | ALTO       | Sensores traseira → segurança |
| 0x02300000 | ESP32-S_02   | RaspberryPi   | DATA   | ALTO       | Sensores traseira → RPi       |
| 0x02400000 | ESP32-S_03   | Difusão       | DATA   | ALTO       | Sensores esquerda             |
| 0x02500000 | ESP32-S_04   | Difusão       | DATA   | ALTO       | Sensores direita              |
| 0x02600000 | ESP32-S      | Difusão       | STATE  | MÉDIO      | Estado ESP32-S                |
| 0x02E00000 | ESP32-S      | Difusão       | HEART  | MÉDIO      | Heartbeat ESP32-S             |
| 0x03000000 | ESP32-A_01   | Difusão       | DATA   | ALTO       | Atuadores frente              |
| 0x03040000 | ESP32-A_01   | ESP32-FS      | DATA   | ALTO       | Atuadores frente → segurança  |
| 0x03100000 | ESP32-A_01   | RaspberryPi   | DATA   | ALTO       | Atuadores frente → RPi        |
| 0x03200000 | ESP32-A_02   | Difusão       | DATA   | ALTO       | Atuadores traseira            |
| 0x03600000 | ESP32-A      | Difusão       | STATE  | MÉDIO      | Estado ESP32-A                |
| 0x03E00000 | ESP32-A      | Difusão       | HEART  | MÉDIO      | Heartbeat ESP32-A             |
| 0x04000000 | ESP32-FS     | Difusão       | SAFETY | SUPER_CRIT | Dados de segurança            |
| 0x04100000 | ESP32-FS     | ESP32-S       | CMD    | CRÍTICO    | Comandos para sensores        |
| 0x04200000 | ESP32-FS     | ESP32-A       | CMD    | CRÍTICO    | Inibição ESP32-A              |
| 0x04300000 | ESP32-FS     | RaspberryPi   | DATA   | CRÍTICO    | Estados segurança → RPi       |
| 0x04400000 | ESP32-FS     | Difusão       | EVENT  | SUPER_CRIT | Eventos de segurança          |
| 0x04500000 | ESP32-FS     | Difusão       | SYNC   | CRÍTICO    | Sincronização temporal        |
| 0x04600000 | ESP32-FS     | Difusão       | STATE  | CRÍTICO    | Estado ESP32-FS               |
| 0x04E00000 | ESP32-FS     | Difusão       | HEART  | ALTO       | Heartbeat ESP32-FS            |
...
| 0x45000000 | ESP32-FS     | Difusão       | EVENT  | SUPER_CRIT | Eventos de segurança          |
| 0x46000000 | ESP32-FS     | Difusão       | SYNC   | CRÍTICO    | Sincronização                 |
| 0x50000000 | ESP32-FS_A   | ESP32-FS      | DATA   | SUPER_CRIT | Feedback atuadores emergência |
| 0x54000000 | ESP32-FS_A   | Difusão       | STATE  | SUPER_CRIT | Estado ESP32-FS_A             |
| 0x5E000000 | ESP32-FS_A   | Difusão       | HEART  | SUPER_CRIT | Heartbeat ESP32-FS_A          |
...
| Depuração / diagnóstico | 500 kbps        | 500 kbps             | Operacional |
...
| UART      | ESP32-FS    | Sensores supercríticos | Variável    | GPS, IMU reserva          |
| UART      | ESP32-FS_A  | Atuadores emergência   | Variável    | PWM, sinais digitais      |
| UART      | RaspberryPi | Estação Solo           | 115200      | Telemetria externa        |
...
O CAN ID de difusão (grupo destino = 0x0) é utilizado para mensagens que devem ser recebidas por todos os grupos no bus.
...
constexpr bool isValidCANGroup(uint8_t group) {
    return group <= 0x5; // 0x0 (reservado) a 0x5 (ESP32-FS_A)
}
